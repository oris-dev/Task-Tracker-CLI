...
import { addTask, renameTask, getTaskById, deleteTask } from "../ManageTasks/task-store.js";
const app = express();
...
app.patch("/api/tasks/:id", async (req, res) => {
    ...
    return res.status(200).json({ message: `Task with the id of ${parsedId} was updated` });
});
...
app.delete("api/tasks/:id", async (req, res) => {
    ...
    return res.status(200).json({message: `Deleted task with the id ${task.id}`})
})
